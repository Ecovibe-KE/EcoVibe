...
# Test Case Pair 1: User List Operations
...
def test_get_users_success_super_admin(client, session):
    """Test super admin can retrieve all users including other admins"""
    super_admin = create_admin_user(session, "super@test.com", Role.SUPER_ADMIN)
    admin = create_admin_user(session, "admin2@test.com", Role.ADMIN)
    user = create_test_user(session)

    login_res = client.post(
        "/api/login",
        data=json.dumps({"email": super_admin.email, "password": "Password123"}),
        content_type="application/json",
    )
    access_token = login_res.get_json()["access_token"]
    ...
    assert response.status_code == 200
    assert data["status"] == "success"
    assert len(data["data"]["users"]) == 3  # super_admin + admin + user


# Test Case Pair 2: User Creation - Success Cases
@patch('utils.mail_templates.send_invitation_email')
def test_create_user_success_admin(mock_send_email, client, session):
    ...
    assert data["status"] == "success"
    assert data["message"] == "User created successfully. Invitation email sent."
    assert mock_send_email.called


@patch('utils.mail_templates.send_invitation_email')
def test_create_user_success_super_admin(mock_send_email, client, session):
...
# Test Case Pair 3: User Creation - Validation Errors
...
# Test Case Pair 4: User Creation - Permission Errors
...
# Test Case Pair 5: User Retrieval Operations
...
# Test Case Pair 6: User Update Operations
def test_update_user_success(client, session):
    ...
    assert data["data"]["industry"] == "Updated Industry"


def test_update_user_permission_denied(client, session):
    ...
    response = client.patch(
        f"/api/user-management/{super_admin.id}",
        data=json.dumps(payload),
        content_type="application/json",
        headers={"Authorization": f"Bearer {access_token}"},
    )
    ...
    assert response.status_code == 403
    assert data["status"] == "error"
    assert "Cannot modify" in data["message"]


# Test Case Pair 7: User Deletion Operations
...
# Test Case Pair 8: User Status Operations
...
# Test Case Pair 9: Role-based Access Control
...
def test_super_admin_can_update_admin(client, session):
    """Test super admin can update admin users"""
    super_admin = create_admin_user(session, "super@test.com", Role.SUPER_ADMIN)
    admin = create_admin_user(session, "admin2@test.com", Role.ADMIN)
    ...
    payload = {
        ...
        "role": "admin"  # Super admin can change admin roles
    }

    response = client.patch(
        f"/api/user-management/{admin.id}",
        data=json.dumps(payload),
        content_type="application/json",
        headers={"Authorization": f"Bearer {access_token}"},
    )
...
# Test Case Pair 10: Edge Cases
def test_update_user_duplicate_data(client, session):
    """Test updating user with duplicate email/phone"""
    admin = create_admin_user(session)
    ...
    response = client.patch(
        f"/api/user-management/{user2.id}",
        data=json.dumps(payload),
        content_type="application/json",
        headers={"Authorization": f"Bearer {access_token}"},
    )
...
def test_partial_update_user(client, session):
    ...
    assert response.status_code == 200
    assert data["status"] == "success"
    assert data["data"]["industry"] == "New Industry"
