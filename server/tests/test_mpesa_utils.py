...
os.environ['FLASK_MPESA_STK_PUSH_URL'] = 'https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest'
os.environ['FLASK_MPESA_QUERY_URL'] = 'https://sandbox.safaricom.co.ke/mpesa/stkpushquery/v1/query'
...
class TestMpesaUtility:
    ...
    def test_generate_password(self):
        ...
        password, timestamp = self.mpesa_utility.generate_password(business_shortcode, passkey)
    ...
    @patch('utils.mpesa_utils.MpesaTokenManager.get_token')
    @patch('utils.mpesa_utils.requests.post')
    def test_check_transaction_status_success(self, mock_post, mock_get_token):
        ...
        result = self.mpesa_utility.check_transaction_status('ws_CO_191220191020363925')
    ...
    @patch('utils.mpesa_utils.MpesaTokenManager.get_token')
    @patch('utils.mpesa_utils.requests.post')
    def test_check_transaction_status_failure(self, mock_post, mock_get_token):
        """Test transaction status check failure"""
        mock_get_token.return_value = 'test_token'
        ...
        mock_response.json.return_value = {
            'ResponseCode': '1',
            'ResponseDescription': 'Failed to query transaction'
        }
        mock_post.return_value = mock_response
    ...
    def test_get_mpesa_credentials(self):
        """Test getting MPESA credentials"""
        credentials = self.mpesa_utility.get_mpesa_credentials()

        assert credentials['business_shortcode'] == '174379'
        assert credentials['passkey'] == 'test_passkey'
        assert credentials['callback_url'] == 'https://example.com/callback'
        assert credentials['timeout'] == 30

    def test_get_mpesa_credentials_missing(self):
        """Test getting MPESA credentials when some are missing"""
        original_shortcode = os.environ.pop('FLASK_MPESA_BUSINESS_SHORTCODE', None)

        credentials = self.mpesa_utility.get_mpesa_credentials()

        assert credentials['business_shortcode'] is None
        assert credentials['passkey'] == 'test_passkey'
...
class TestMpesaUtilityGlobalInstance:
    """Test the global mpesa_utility instance"""

    def test_global_instance(self):
        """Test that global instance is created"""
        assert isinstance(mpesa_utility, MpesaUtility)
        assert hasattr(mpesa_utility, 'token_manager')
        assert hasattr(mpesa_utility, 'initiate_stk_push')