...
describe("Terms Component", () => {
  ...
  // Replace snapshot test with comprehensive structure test
  test("renders all main sections in correct order", () => {
    const headings = screen.getAllByRole('heading');

    // Check that all main section headings are present and in correct order
    const headingTexts = headings.map(heading => heading.textContent);

    expect(headingTexts).toContain('Terms and Conditions');
    expect(headingTexts).toContain('Introduction');
    expect(headingTexts).toContain('Use of Our Services');
    expect(headingTexts).toContain('Consulting Services');
    expect(headingTexts).toContain('Intellectual Property');
    expect(headingTexts).toContain('Limitation of Liability');
    expect(headingTexts).toContain('Indemnification');
    expect(headingTexts).toContain('Termination');
    expect(headingTexts).toContain('Governing Law');
    expect(headingTexts).toContain('Changes to These Terms');
  });


  test("renders all sections with proper semantic structure", () => {
